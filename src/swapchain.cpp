...
Swapchain::Swapchain(VkDevice device, VkPhysicalDevice physical_device, VkSurfaceKHR surface, VkExtent2D desired_extent, VSyncMode v_sync) {
    ...
    swapchain_extent = desired_extent;
    ...
    if(swapchain_extent.width != desired_extent.width || swapchain_extent.height != desired_extent.height)
