...
void DebugPass::init(
    ...
    m_sphere_mesh_vertex_buffer = api.m_resource_manager->create_buffer(BufferCreateInfo{
    ...
    m_sphere_mesh_index_buffer = api.m_resource_manager->create_buffer(BufferCreateInfo{
        .size = sphere_mesh_index_buffer_size,
        .buffer_usage_flags = VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT,
        .memory_usage_flags = VMA_MEMORY_USAGE_GPU_ONLY
    });

    Handle<Buffer> staging = api.m_resource_manager->create_buffer(BufferCreateInfo{
    ...
    api.m_resource_manager->memcpy_to_buffer_once(staging, sphere_mesh_positions.data(), sphere_mesh_vertex_buffer_size);
    ...
    api.m_resource_manager->memcpy_to_buffer_once(staging, sphere_mesh_indices.data(), sphere_mesh_index_buffer_size);
    ...
    api.m_resource_manager->destroy_buffer(staging);

    m_graphics_descriptor = api.m_resource_manager->create_descriptor(DescriptorCreateInfo{
        .bindings {
    ...
    api.m_resource_manager->update_descriptor(m_graphics_descriptor, DescriptorUpdateInfo{
        .bindings{
    ...
    m_render_target = api.m_pipeline_manager->create_render_target(m_graphics_pipeline, RenderTargetCreateInfo{
        .color_target_handle = offscreen_image,
        .depth_target_handle = depth_image
    });

    m_sphere_indirect_draw_buffer = api.m_resource_manager->create_buffer(BufferCreateInfo{
    ...
    m_compute_descriptor = api.m_resource_manager->create_descriptor(DescriptorCreateInfo{
        .bindings{
    ...
    api.m_resource_manager->update_descriptor(m_compute_descriptor, DescriptorUpdateInfo{
        .bindings {
            DescriptorBindingUpdateInfo {
                .binding_index = 0u,
                .buffer_info {
...
void DebugPass::destroy(const RenderAPI &api) {
    api.m_pipeline_manager->destroy_compute_pipeline(m_compute_pipeline);
    api.m_resource_manager->destroy_descriptor(m_compute_descriptor);

    api.m_resource_manager->destroy_buffer(m_sphere_indirect_draw_buffer);
    api.m_pipeline_manager->destroy_render_target(m_render_target);
    api.m_pipeline_manager->destroy_graphics_pipeline(m_graphics_pipeline);
    api.m_resource_manager->destroy_descriptor(m_graphics_descriptor);

    api.m_resource_manager->destroy_buffer(m_sphere_mesh_vertex_buffer);
    api.m_resource_manager->destroy_buffer(m_sphere_mesh_index_buffer);
}

void DebugPass::resize(const RenderAPI &api, Handle<Image> offscreen_image, Handle<Image> depth_image) {
    api.m_pipeline_manager->destroy_render_target(m_render_target);
...
void DebugPass::process(const RenderAPI &api, Handle<CommandList> cmd, f32 debug_shape_opacity) {
    ...
    api.begin_graphics_pipeline(cmd, m_graphics_pipeline, m_render_target, RenderTargetClear{});
        api.push_graphics_constants(cmd, m_graphics_pipeline, &debug_shape_opacity);
        api.bind_graphics_descriptor(cmd, m_graphics_pipeline, m_graphics_descriptor, 0u);
